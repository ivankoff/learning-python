...
device_list = str( list( london_co.keys() ) )
device_name = ( input( 'Enter device name ' + device_list + ': ' ) ).strip().lower()

parameter_list = str( list( london_co[ device_name ].keys() ) )
parameter_name = ( input( 'Enter parameter name ' + parameter_list + ': ' ) ).strip().lower()

print( london_co[ device_name ].get( parameter_name, 'Wrong parameter!' ) )
